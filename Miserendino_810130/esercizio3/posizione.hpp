  ...
  class corpoceleste : public particella {
      ...
      corpoceleste(std::string nome, double massa, double raggio) : particella(massa,0)
    {
      ...
      void setnome(std::string nome){m_nome=nome;}
       double calcolapotenziale(posizione);       // per usare questo metodo dovrei avere la posizione come se l'universo fosse un sistema di riferimento cartesiano
      double calcolapotenziale(double);
      ...
      posizione getposizione(){return r;}
      double getxposizione(){return r.getx();}
      double getyposizione(){return r.gety();}
      double getzposizione(){return r.getz();}
      void setposizione(double x,double y,double z){r.setx(x);r.sety(y);r.setz(z);}
      virtual void print();
    protected:
      std::string m_nome;
      posizione r;
    ...
    double corpoceleste::calcolapotenziale(double posizione)
      {
        return (-(G*m_massa)/posizione);
      }  
