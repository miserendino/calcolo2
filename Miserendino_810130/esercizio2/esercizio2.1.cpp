...
ifstream input;             // il numero degli elementi sono il primo dato del file!!!
/*
double tmp;
input >> tmp;
    while(!input.eof())
  {
    ...
    */
    input.open(argv[1]);
    unsigned int number_of_elements;   // il numero degli elementi sono il primo numero di input del file!!!
...
// #endif
  conta++;
  conta2++;
  std::cout << "Numero degli scambi tra gli elementi del vettore (variabile non statica) " << conta << std::endl;
  std::cout << "Numero degli scambi tra gli elementi del vettore (variabile statica) " << conta2 <<  std::endl;
  v[i+1]=v[i];
    v[i]=pippo;
   }
   else continue;
