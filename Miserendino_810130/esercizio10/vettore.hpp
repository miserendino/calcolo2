...
public:
  vettore ()
  {
  }				// bisogna mettere le parentesi altrimenti il compilatore frigna          
  vettore (unsigned int size)
  {
    ...
    for (unsigned int i = 0; i < size; ++i)
      {
	m_v[i] = 0;
  }}				//inizializzo tutti gli elementi del vettore con 0
...
unsigned int
vettore::setcomponent (unsigned int position, double value)
{
  while (position > m_size)
    {
      std::
	cerr <<
	"the value specified is bigger than the lenght of the array,exiting"
	<< std::endl;
      return -1;
    }
  m_v[position] = value;
...
double
vettore::getcomponent (unsigned int position)
{
  while (position > m_size)
    {
      std::
...
vettore::vettore (vettore & pluto)
{
  ...
  for (unsigned int i = 0; i < m_size; ++i)
    {				// assegno i valori di ciascuna componente del vettore nuovo
      m_v[i] = pluto.m_v[i];
    }
}
...
private:
  unsigned int
    num = 0;			// numero degli elementi che voglio carica e utilizzare nel mio vettore di dati
  char
    namefile[32];
  double *
    v;
...
void
vettoredati::caricavett ()
{
  load (v, num);
}

void
vettoredati::quicksort ()
{
  quicksort1 (v, num);
}

void
vettoredati::print (char type[32])
{
  write_in_file (v, num);
}
...
void
vettoredati::varianza ()
{
  calcola_varianza (v, num);
}

void
vettoredati::deviazione_standard ()
{
